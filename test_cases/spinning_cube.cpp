...
unique_ptr<Cube> spinning_cube()
{
    ...
    auto& cube = *_cube;
    mat3 a = skew(omega);
    cube.dqdt.block<3, 3>(0, 1) = a;
    cube.q.col(0) = vec3(0.0, 1.0, 0.0);
    cube.q0 = cube.q;
    return _cube;
}
...
void cube_blocks(int n)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        auto& cube = *_cube;
        auto R = skew(omega);
        cube.dqdt.block<3, 3>(0, 1) = R;
        cube.dqdt.col(0) = vec3(0.0, i * -5.0, 0.0);

        cube.q.col(0) = vec3(i * 0.5, 1.0 * (i * 1.5 + 0.1), i * 0.5);
        cube.q0 = cube.q;
...
void customize(string file)
{
    ...
    for (auto &it: data) {
        ...
        a.dqdt.col(0) = vec3(p_dot[0], p_dot[1], p_dot[2]);
        a.q.col(0) = vec3(p[0], p[1], p[2]);
        auto R = skew(vec3(omega[0], omega[1], omega[2]));
        a.dqdt.block<3, 3>(0, 1) = R;
        if (use_euler) {
            auto ds = it["euler"];
