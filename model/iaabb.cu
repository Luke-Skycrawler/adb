...
__global__ void ee_toi_decision_kernel(
    ...
    for (int _i = 0; _i < n_task_per_thread; _i ++) {
        ...
        if (idx < n_tasks) {
            ...
            int i= ij[0];
            int j = ij[1];
            Edgef eit1 {eiejs[i * 4], eiejs[i * 4 + 1]};
            Edgef eit2 {eiejs[i * 4 + 2], eiejs[i * 4 + 3]};
            Edgef ejt1 {eiejs[j * 4 + nei], eiejs[j * 4 + 1 + nei]};
            Edgef ejt2 {eiejs[j * 4 + 2 + nei], eiejs[j * 4 + 3 + nei]};
    ...
    int _nvi)
{
    ...
    for (int idx = 0; idx < *cnt; idx++) {
        ...
        auto t = pt_collision_time(vt1, ft1, vt2, ft2);
        toi = CUDA_MIN(toi, t);
    }
    return toi;
}
...

{
    ...
    auto stashed_lt_back = g.leader_thread_buffer_back;
    auto &lt_back { g.leader_thread_buffer_back };
    ...
    if (vtn == 1) {
        ...
        g.nee_line_search = g.npt_line_search = 0;
    }
        ...
        if (I < T_t2) {
            ...
            case 2: {
                ...
                if (vtn != 3) {
                    ...
                    joint_aabbs[T_t +  I] = merge(compute_aabb(e, 0.0f), compute_aabb(e2, 0.0f));
                }
                break;
            }
            }
...
#ifdef CPU_REF
void prepare_vifj(
    i2 ij, vector<cudaAffineBody>& cubes, int type, int vtn,
    ...
    vector<vec3f>& vifjs, vector<luf>& joint_aabbs)
{
    ...
    if (vtn == 3) size_vifj *= 2;
    vifjs.resize(size_vifj);
    ...
    for (int I = 0; I < T_2; I++) {
        ...
        case 0: {
            ...
            if (vtn != 3) {
                ...
                vifjs[T * 2 + I * 6] = f.t0;
                vifjs[T * 2 + I * 6 + 1] = f.t1;
                vifjs[T * 2 + I * 6 + 2] = f.t2;
                auto& f2 = cj.triangle_updated(i);
                vifjs[T * 2 + I * 6 + 3] = f2.t0;
                vifjs[T * 2 + I * 6 + 4] = f2.t1;
                vifjs[T * 2 + I * 6 + 5] = f2.t2;
                joint_aabbs[T + I] = merge(compute_aabb(f, 0.0f), compute_aabb(f2, 0.0f));
            }
            break;
        }
        case 2: {
            ...
            if (vtn != 3) {
                ...
                vifjs[T * 4 + I * 4] = e.e0;
                vifjs[T * 4 + I * 4 + 1] = e.e1;
                auto& e2 = cj.edge_updated(i);
                vifjs[T * 4 + I * 4 + 2] = e2.e0;
                vifjs[T * 4 + I * 4 + 3] = e2.e1;
                joint_aabbs[T + I] = merge(compute_aabb(e, 0.0f), compute_aabb(e2, 0.0f));
            }
            break;
        }
        }
...
void per_intersection_core(int n_overlaps, luf* culls, i2* overlaps, int vtn, float* toi, float *E_barrier)
{
    ...
    *E_barrier = 0;
    ...
    vec3f *host_projected, *host_updated, *host_vertices;
    int *host_edges, *host_faces;
...
#pragma omp parallel for schedule(guided)
    for (int i = 0; i < n_overlaps; i++) {
        ...
        char *st_back_stashed{ st }, *bulk_back_stashed{ bulk };

        vector<int> host_prmts[MAX_TYPES][2];
        int* meta_sizes = (int*)st;
        st += 6 * (sizeof(int));
        for (int type = 0; type < MAX_TYPES; type++) {
            // designate buffers

            int* ret_meta_sizes = 2 * type + meta_sizes;
        ...
        float host_toi_by_type[3];
#endif
        int nvifj, nvjfi, neiej;
        int *host_cnts_ptr[3], cnt[3];
        ...
        for (int type = 0; type < MAX_TYPES; type++) {
            ...
            case 1:
                ...
                v_offset = sizes[0];
                _flist = flist;
                _vlist = vlist + v_offset;
                break;
            case 2:
                nvi = sizes[4];
                nfj = sizes[5];
                f_offset = sizes[4];
                _flist = elist + f_offset;
                _vlist = elist;
                break;
            }
...
#ifdef CPU_REF
            {
                prepare_vifj(ij, host_cuda_globals.host_cubes,
                    type, vtn,
                    type == 2? host_prmts[2][0]: host_prmts[0][0], 
                    type == 2? host_prmts[2][1]: host_prmts[0][1],

                    type == 2? host_prmts[2][0]: host_prmts[1][0], 
                    type == 2? host_prmts[2][1]: host_prmts[1][1],
                ...
                if (vtn == 3) vf_cnt *= 2; // vt1 and vt2
                thrust::device_vector<vec3f> d_vifjs(vifjs, vifjs + vf_cnt);
            ...
            if (vtn != 3) {
                if (type < 2) {
                    ...
                    // edge_edge
                    ee_filter_distance_kernel_atomic<<<1 , n_cuda_threads_per_block, 0, stream>>>(buf_ij, ret_ij, ret_cnt, vifjs, _vlist, _flist, pt_types, 1e-4f, nvi);
                }
                CUDA_CALL(cudaGetLastError());
                CUDA_CALL(cudaStreamSynchronize(stream));
                ...
                CUDA_CALL(cudaStreamSynchronize(stream));
                #ifdef CPU_REF
                float t = toi_decision_host(host_culled_pairs.data(), &host_ret_cnt, host_vifjs.data(), nvi);
                host_toi_by_type[type] = t;
                #endif
            }
        } // end of primtive pairwise loop

        if (vtn != 3) {
            // FIXME: for vtn == 2, don't put collision set directly into the global variable
            CUDA_CALL(cudaStreamSynchronize(stream));
            CUDA_CALL(cudaMemcpyAsync(cnt, host_cnts_ptr[0], sizeof(int) * 3, cudaMemcpyDeviceToHost, stream));
            CUDA_CALL(cudaStreamSynchronize(stream));
            ...
            auto &npt {vtn == 2 ? host_cuda_globals.npt_line_search: host_cuda_globals.npt};
            auto &nee {vtn == 2 ? host_cuda_globals.nee_line_search: host_cuda_globals.nee};
            i2 *ptp {vtn == 2 ? host_cuda_globals.pt_line_search.p: host_cuda_globals.pt.p};
            i2 *ptb {vtn == 2 ? host_cuda_globals.pt_line_search.b: host_cuda_globals.pt.b};
            i2 *eep {vtn == 2 ? host_cuda_globals.ee_line_search.p: host_cuda_globals.ee.p};
            i2 *eeb {vtn == 2 ? host_cuda_globals.ee_line_search.b: host_cuda_globals.ee.b};
...
#pragma omp critical
            ...
            cudaMemcpyAsync(ptp + pt_put, vifj_ptr, nvifj * sizeof(i2), cudaMemcpyDeviceToDevice, stream);
            cudaMemcpyAsync(ptp + pt_put + nvifj, vjfi_ptr, nvjfi * sizeof(i2), cudaMemcpyDeviceToDevice, stream);
#ifndef PT_ONLY
            cudaMemcpyAsync(eep + ee_put, eiej_ptr, neiej * sizeof(i2), cudaMemcpyDeviceToDevice, stream);
#endif
            strided_memset_kernel<<<1, n_cuda_threads_per_block, 0, stream>>>(ptb + pt_put, ij, nvifj);
            strided_memset_kernel<<<1, n_cuda_threads_per_block, 0, stream>>>(ptb + pt_put + nvifj, ji, nvjfi);
#ifndef PT_ONLY
            ...
            // vtn == 3, ccd
            cudaMemcpyAsync(toi_by_type, ret_tois, sizeof(float) * 3, cudaMemcpyDeviceToHost, stream);
            CUDA_CALL(cudaStreamSynchronize(stream));
            #ifdef PT_ONLY
            ...
            #pragma omp critical
            {
                *toi = std::min(*toi, toi_per_overlap);
            }
        }

        {
            // recover small temporary buffer
            bulk = bulk_back_stashed;
            st = st_back_stashed;
        }
    }
    CUDA_CALL(cudaDeviceSynchronize());
